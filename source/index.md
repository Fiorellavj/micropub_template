...
```{figure} figures/micropublication/composite_figure.png
:name: figure-main
...
\
**a.** Example detection of a compound event at a single pixel during 1998, showing daily maximum and minimum temperatures smoothed using a moving window of 5 days (top) and daily pentad-SPEI (bottom). Heatwaves (orange shading) and flash droughts (blue shading) are detected using the respective thresholds expressed as the N-th percentiles (horizontal lines) in accordance with Fu & Wang (2022); compound events (purple hatched shading) are periods of temporal overlap between concurrent heatwave and flash drought events.
\
**b.** Annual maximum severity of heatwaves (top, orange), flash droughts (middle, blue; y-axis inverted because the severity values are negative), and compound events (bottom, purple) across Peru over the study period.
\
